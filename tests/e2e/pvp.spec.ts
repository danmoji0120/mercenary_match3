...
test('two players attack, defend, finish, and rematch', async ({ browser }) => {
  ...
  await a.locator('.loadout-slots button').first().click();
  await a.locator('.roster button').filter({ hasText: '\uD074\uB77C\uB9AC\uC2A4' }).click();
  await expect(a.getByTestId('character-detail')).toContainText('\uC561\uD2F0\uBE0C');
  await a.getByTestId('character-detail').getByRole('button', { name: /\uC804\uD22C\uC6D0\uC5D0 \uBC30\uCE58/ }).click();
  await a.getByRole('button', { name: '\uD3B8\uC131 \uC801\uC6A9' }).click();
  await a.reload();
  ...
  await expect(b.locator('.combatant-header.opponent')).toContainText('\uD074\uB77C\uB9AC\uC2A4');
