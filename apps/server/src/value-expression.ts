...
function resultValue(result: EffectResult | undefined, path: string): number | boolean {
  if (!result) throw new Error('VALUE_RESULT_UNAVAILABLE');
  const value = result[path as keyof EffectResult];
  if (typeof value !== 'number' && typeof value !== 'boolean') throw new Error(`VALUE_RESULT_PATH_UNAVAILABLE:${path}`);
  return value;
}
