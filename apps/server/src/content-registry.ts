...
import { TRIGGER_TYPES, type AbilityDefinition, type ConditionDefinition, type EffectDefinition, type JsonValue, type StatusDefinition, type ValueExpression } from './effect-types.js';

const trigger = z.object({ type: z.enum(TRIGGER_TYPES), leadTimeMs: z.number().int().nonnegative().optional() }).strict();
const valueExpression: z.ZodType<ValueExpression> = z.lazy(() => z.union([
  z.object({ type: z.literal('CONSTANT'), value: z.union([z.number().finite(), z.boolean(), z.string()]) }).strict(),
  z.object({ type: z.literal('STAT'), target: z.enum(['SELF', 'ENEMY']), stat: z.literal('MAX_HP') }).strict(),
  z.object({ type: z.literal('RESOURCE'), target: z.enum(['SELF', 'ENEMY']), resource: z.enum(['HP', 'HP_RATIO', 'SHIELD', 'MANA']) }).strict(),
  z.object({ type: z.literal('EVENT_VALUE'), path: z.string().min(1) }).strict(),
  z.object({ type: z.literal('RESULT_VALUE'), resultKey: z.string().min(1), path: z.string().min(1) }).strict(),
  z.object({ type: z.enum(['ADD', 'SUBTRACT', 'MULTIPLY', 'DIVIDE', 'MIN', 'MAX']), values: z.array(valueExpression).min(2) }).strict(),
  z.object({ type: z.literal('CLAMP'), value: valueExpression, min: valueExpression, max: valueExpression }).strict(),
  z.object({ type: z.enum(['ABS', 'FLOOR', 'CEIL', 'ROUND']), value: valueExpression }).strict(),
]));
const jsonValue: z.ZodType<JsonValue> = z.lazy(() => z.union([z.null(), z.boolean(), z.number().finite(), z.string(), z.array(jsonValue), z.record(jsonValue)]));
const condition: z.ZodType<ConditionDefinition> = z.lazy(() => z.object({ type: z.string().min(1), operator: z.enum(['eq', 'neq', 'gt', 'gte', 'lt', 'lte']).optional(), value: z.union([z.number(), z.boolean(), z.string()]).optional(), field: z.string().optional(), tag: z.string().optional(), statusId: z.string().optional(), tileType: z.enum(['SWORD', 'SHIELD', 'HEAL', 'MANA']).optional(), effectKey: z.string().optional(), direction: z.enum(['upward', 'downward']).optional(), conditions: z.array(condition).optional(), condition: condition.optional(), left: valueExpression.optional(), right: valueExpression.optional() }).strict());
const effect: z.ZodType<EffectDefinition> = z.lazy(() => z.object({ type: z.string().min(1), key: z.string().optional(), target: z.enum(['self', 'opponent']).optional(), amount: z.union([z.number(), valueExpression]).optional(), ratio: z.union([z.number(), valueExpression]).optional(), maximum: z.number().optional(), durationMs: z.number().int().nonnegative().optional(), statusId: z.string().optional(), delayMs: z.number().int().nonnegative().optional(), effects: z.array(effect).optional(), elseEffects: z.array(effect).optional(), conditions: z.array(condition).optional(), message: z.string().optional(), scope: z.literal('chain_step').optional(), cap: z.number().optional(), tags: z.array(z.string()).optional(), travelMs: z.number().int().nonnegative().optional(), shieldBypassRatio: z.union([z.number().min(0).max(0.5), valueExpression]).optional(), initialCharges: z.number().int().nonnegative().optional(), flag: z.string().optional(), value: z.union([z.number(), z.boolean(), z.string()]).optional(), resource: z.enum(['HP', 'SHIELD', 'MANA']).optional(), allowPartial: z.boolean().optional(), canReduceHpBelowOne: z.boolean().optional(), handlerId: z.string().optional(), parameters: jsonValue.optional() }).strict());
const abilitySchema = z.object({ id: z.string().regex(/^[a-z0-9_]+$/), kind: z.enum(['active', 'support']), name: z.string().min(1), shortDescription: z.string().min(1), fullDescription: z.string().min(1), trigger, cost: z.number().int().min(0).max(100), cooldownMs: z.number().int().nonnegative(), oncePerBattle: z.boolean(), maxTriggersPerBattle: z.number().int().positive().nullable(), chainLimit: z.object({ maxTriggers: z.number().int().positive() }).strict().nullable().default(null), conditions: z.array(condition), effects: z.array(effect).min(1), tags: z.array(z.enum(['offense', 'defense', 'heal', 'disruption', 'shield', 'execute'])), contentVersion: z.number().int().positive(), enabled: z.boolean() }).strict();
...
export function loadContentRegistry(root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../../content/core'), generatedAbilities?: AbilityDefinition[]) {
  const abilities = generatedAbilities ?? jsonFiles(path.join(root, 'abilities')).map((file) => JSON.parse(readFileSync(file, 'utf8')) as AbilityDefinition);
