...
describe('authoritative battle', () => {
  ...
  it('applies the configured shield bypass before the remaining queued damage', () => { const { battle } = make(); battle.player('a')!.gauge = 100; battle.useSkill('a', 'skill'); const attack = battle.pendingAttacks[0]!; applyShield(battle.player('b')!, 200); battle.tick(attack.arrivesAt); expect(battle.player('b')!.hp).toBe(946); expect(battle.player('b')!.shield).toBe(74) });
