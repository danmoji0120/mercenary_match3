...
describe('frenzy lifecycle', () => {
  ...
  it('applies frenzy values to match effects and skill attacks', () => { const { battle } = make(); battle.tick(battle.endsAt - 30_000); const value = resolution([{ type: 'SWORD', count: 3 }, { type: 'SHIELD', count: 3 }, { type: 'HEAL', count: 3 }, { type: 'MANA', count: 3 }]); battle.player('a')!.hp = 900; apply(battle, 'a', value); expect(value.effects.map((effect) => effect.amount)).toEqual([95, 52, 18, 20]); battle.player('a')!.gauge = 100; battle.useSkill('a', 'skill'); expect(battle.pendingAttacks.at(-1)?.damage).toBe(257) });
});
...
describe('authoritative battle statistics', () => {
  ...
  it('separates generated, shield, HP, received, blocked, and shield-break damage', () => { const { battle } = make(); battle.player('b')!.shield = 70; battle.player('a')!.gauge = 100; battle.useSkill('a', 'skill'); const attack = battle.pendingAttacks[0]!; battle.tick(attack.arrivesAt); expect(battle.stats.a).toMatchObject({ totalDamageGenerated: 190, shieldDamageDealt: 70, hpDamageDealt: 120, skillUseCount: 1, attacksQueued: 1 }); expect(battle.stats.b).toMatchObject({ totalDamageReceived: 190, hpDamageReceived: 120, damageBlockedByShield: 70, shieldBreakCount: 1 }) });
  it('counts fully blocked attacks and prevents duplicate resolution', () => { const { battle } = make(); battle.player('b')!.shield = 500; battle.player('a')!.gauge = 100; battle.useSkill('a', 'skill'); const attack = battle.pendingAttacks[0]!; battle.tick(attack.arrivesAt); const frozen = { ...battle.stats.b }; battle.tick(attack.arrivesAt + 1); expect(battle.stats.b.attacksFullyBlocked).toBe(1); expect(battle.stats.b).toEqual(frozen) });
