export const APP_TABS = ['mercenaries', 'lobby'] as const;
export type AppTab = (typeof APP_TABS)[number];

const paths: Record<AppTab, string> = { mercenaries: '/mercenaries', lobby: '/lobby' };
...
export type MercenaryRoute = { kind: 'collection' } | { kind: 'detail'; characterId: string } | { kind: 'loadout' };
export function mercenaryRouteForPath(pathname: string): MercenaryRoute | null {
  ...
  const match = normalized.match(/^\/mercenaries\/([^/]+)$/); if (!match) return null;
  try { return { kind: 'detail', characterId: decodeURIComponent(match[1]!) } } catch { return null }
}
export function mercenaryDetailPath(characterId: string) { return `/mercenaries/${encodeURIComponent(characterId)}` }
