...
function LobbyCharacterDisplay({ account }: { account: UserAccountState }) {
  ...
  return <section className="lobby-character-display" aria-label="현재 출전 용병">
    ...
    <figure className="lobby-lead"><CharacterPortrait src={loadout.combatant.portraitAsset} alt=""/><figcaption><small>전투원</small><strong>{loadout.combatant.name}</strong><span>{loadout.combatant.rarity}</span></figcaption></figure>
    {loadout.supports.map((support, index) => <figure className={`lobby-support support-${index + 1}`} key={support.characterId}><CharacterPortrait src={support.portraitAsset} alt=""/><figcaption><small>지원 {index + 1}</small><strong>{support.name}</strong></figcaption></figure>)}
  </section>;
}
